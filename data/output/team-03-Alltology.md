...
데이터베이스 관련 기술, 온톨로지, AI 모델등에 관심있다. 겨울방학동안 이공계 학생들을 위한 LLM 및 노트관리 서비스 프로젝트에 참여하여 배포까지 하고 데모데이를 열어서 박람회에 참가한 경험이 있다.
...
프론트엔드, 백엔드보다는 통계학 지식을 베이스로 한 AI 자체에 더 관심이 많았기 때문에 AI 모델을 만드는 경험이 많습니다. 다수의 공모전에 참여했고, 예측 모델을 만드는 공모전에서 시계열 지식 기반의 모델을 만들어 본 경험이 있습니다. 그 외에도 Computer Vision 수업에서 사람 이미지에 옷 이미지를 합성해주는 서비스를 개발해 발표한 경험이 있습니다.
...
1. 대형 언어모델의 성능 향상을 위해, 모델 내부 구조를 변경하는 접근(예: 파인튜닝, LoRA 등)과 외부 구조를 활용하는 접근(예: RAG, Knowledge Augmentation)은 각각 어떤 특성과 한계를 가지는가?
...
1. 지식 충돌 문제: 모델이 미리 배운 내용(FT)과 새로 검색해온 내용(RAG)이 다를 때, 어떤 정보를 우선하도록 설계해야 하는가?
