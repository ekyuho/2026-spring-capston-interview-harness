...
import generate_team_dossiers_v14 as v14

QUESTION_GUIDE_PATTERN = re.compile(r"^\s*질문당\s*한줄씩\s*5개의\s*질문\s*:\s*", flags=re.IGNORECASE)
...
def normalize_five_questions(items: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for item in items:
        ...
        content = QUESTION_GUIDE_PATTERN.sub("", content.strip())
        content = re.sub(r"\n\s*질문당\s*한줄씩\s*5개의\s*질문\s*:\s*", "\n", content, flags=re.IGNORECASE)
...
def render_team_dossier(context: dict) -> str:
    context = {
        **context,
        "five_questions": normalize_five_questions(context.get("five_questions", [])),
    }

    supplement_summary = v14.v8.format_supplement_text_v8(context.get("supplement_summary", ""))
    ...
    base_software = v14.format_base_software(context.get("base_software", ""))
...
{v14.v8.base.render_member_section(context.get('member_surveys', []), '자료 없음 또는 column_map.yaml 보정 필요')}

## 5. 개인별 5 Questions

{render_member_section_clean(context.get('five_questions', []), '자료 없음 또는 column_map.yaml 보정 필요')}
...
if __name__ == "__main__":
    ...
    v14.v8.split_sentences_korean = v14.split_sentences_korean_fixed
    v14.v8.format_field_block_v8 = v14.format_field_block_v14
    ...
    v14.v8.base.render_team_dossier = render_with_advisor
    v14.v8.base.main()
