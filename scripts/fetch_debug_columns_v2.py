...
from source_config import (
    ROOT,
    github_raw_url_to_repo_relative_path,
    load_sources,
...
def github_pdf_folder_summary(source_key: str, source: dict[str, Any]) -> str:
    local_dir = resolve_github_pdf_folder_local_path(source_key)
    extension = pdf_extension_for_source(source_key)
    repo_relative = github_raw_url_to_repo_relative_path(source["base_url"])

    if not local_dir.exists():
        return (
            f"- local_dir: `{local_dir}`\n"
            f"- repo_relative_path: `{repo_relative}`\n"
            f"- 상태: 로컬 폴더가 없어 아직 읽을 수 없음\n"
        )
...
def main() -> None:
    ...
    for source_key, source in sources.items():
        ...
        try:
            ...
            elif source_type == "github_pdf_folder":
                report.append(github_pdf_folder_summary(source_key, source))
                report.append("\n")
